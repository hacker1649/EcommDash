...
<head>
  ...
  <title>User Login Basic - Pages | Sneat - Bootstrap 5 HTML Admin Template - Pro</title>
...
<body>
  ...
  if (isset($_SESSION['userLoggedIn']) && $_SESSION['userLoggedIn'] === true) {
    // Redirect to website page if user is already logged in
    header("Location: ./website/website_page.php");
    exit;
  }
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($emailErr) && empty($passwordErr)) {
      ...
      $sql = "SELECT user_id, user_email, user_password FROM tbl_user WHERE user_email = '$email'";
      ...
      if ($result->num_rows > 0) {
        ...
        $hashedOldPassword = sha1($password);
        if ($hashedOldPassword === $user['user_password']) {
          // Set session variables for the logged-in user
          $id = $user['user_id'];
          $_SESSION['user_id'] = $user['user_id'];
          $_SESSION['email'] = $user['user_email'];
          $sql = "SELECT first_name, last_name FROM tbl_user_profile WHERE user_id = $id";
          $result = $conn->query($sql);

          if ($result->num_rows > 0) {
            $user_profile = $result->fetch_assoc();
            $_SESSION['username'] = trim($user_profile['first_name'] . " " . $user_profile['last_name']); // Trimming the username
