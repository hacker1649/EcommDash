...
<head>
  ...
  <title>Add Product - Analytics | Sneat - Bootstrap 5 HTML Admin Template - Pro</title>
...
<body>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($productNameErr) && empty($productDescErr) && empty($priceErr) && empty($categoryErr)) {
      ...
      $insert_product = "INSERT INTO tbl_product (product_name, product_price, product_desc, category_id, created_on, product_status) 
      VALUES ('$productName', '$price', '$productDesc', '$category_id', '$current_time', 1)";

      if ($conn->query($insert_product) === TRUE) {
        $_SESSION['success'] = "New product record added successfully...";
